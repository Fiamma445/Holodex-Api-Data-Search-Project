...
데이터 모델도 한 번에 정해지지 않았습니다. 모든 JSON 필드를 테이블로 펼치면 SQL은 편해지지만, Holodex 응답 구조가 조금만 바뀌어도 스키마를 계속 고쳐야 했습니다. 반대로 JSON을 그대로만 저장하면 나중에 분석 쿼리가 복잡해집니다. 그래서 원본 JSON은 보존하고, `mentions`, `topic`, `status`처럼 자주 쓰는 값만 SQLite JSON1로 필요한 순간에 꺼내 쓰는 구조를 선택했습니다.
...
- 단순히 많이 모았다는 말보다, `status`, `topic`, `mentions`처럼 분석에 쓰는 필드가 실제로 얼마나 채워져 있는지를 먼저 확인했습니다.
...
이 서비스는 단순 팬 사이트보다는 멤버별 콘텐츠 흐름을 빠르게 살펴보는 탐색 도구에 가깝습니다. 특정 멤버가 어떤 콘텐츠를 많이 했는지, 어느 시기에 방송량이 늘거나 줄었는지, 누구와 자주 콜라보했는지를 한 번에 볼 수 있습니다. 이런 정보는 콘텐츠 기획 흐름을 보거나 아카이브를 큐레이션할 때 근거로 사용할 수 있습니다.
