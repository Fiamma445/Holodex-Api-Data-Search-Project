...
> Holodex API의 라이브, 아카이브, 노래 메타데이터와 콜라보 관계를 **검색 가능한 로컬 데이터 마트**로 재구성하고, 멤버별 방송 흐름과 콘텐츠 관계를 탐색 가능한 라이브 서비스로 연결한 프로젝트

- Live Service: [https://holo-search.xyz](https://holo-search.xyz/)

## 이 프로젝트가 답하려 한 질문

Holodex는 버튜버 방송 정보를 찾기 좋은 API와 서비스를 제공합니다. 다만 실제 사용자가 원하는 흐름은 단순한 영상 조회 하나로 끝나지 않습니다. 라이브와 예정 방송을 보고, 지나간 아카이브를 조건별로 찾고, 외부 콜라보 관계를 잡고, 노래 DB와 통계까지 한 화면에서 이어서 탐색하려면 여러 API 엔드포인트와 응답 옵션을 하나의 기준으로 묶어야 했습니다.
...
| 분석상의 공백 | 멤버별 장기 흐름, 콜라보 관계, 노래 구간 탐색을 한 서비스에서 연결하기 어려움 |
...
처음에는 프론트엔드에서 Holodex API를 직접 호출하는 방식도 가능했습니다. 하지만 라이브, 영상 목록, 곡 정보, 멘션 관계는 필요한 엔드포인트와 응답 구조가 다르고, API Key 유무나 Rate Limit에 따라 사용자마다 결과가 흔들릴 수 있었습니다. 그래서 공용 기능은 서버 환경변수의 Holodex API Key로 처리하고, 검색과 통계는 SQLite에 적재한 데이터를 기준으로 제공하는 구조를 선택했습니다.
...
운영 단계에서 가장 크게 바뀐 판단은 **API 과다 호출을 줄이기 위해 핵심 기능을 DB 기반으로 분리한 것**입니다. 라이브/예정, 아카이브, 노래 DB, 통계, 채널 인덱스는 서버가 주기적으로 동기화한 SQLite 데이터를 읽습니다. 대용량 seed DB와 채널 아이콘 같은 정적 리소스는 Cloudflare R2 같은 object storage로 빼서, Git 저장소와 Railway 런타임에 무거운 파일을 직접 싣지 않도록 설계했습니다.
...
| API 호출 절감 | 주요 기능은 DB 조회 우선 | 같은 채널/페이지를 볼 때마다 Holodex API를 반복 호출하지 않기 위해 |
...
이 흐름에서 핵심은 반복 조회가 많은 화면을 DB와 정적 리소스 계층으로 분리한 점입니다. 아카이브, 라이브/예정, 노래, 통계, 채널 아이콘은 서버가 보유한 데이터에서 읽어 API 호출을 줄입니다. 즉 사용자는 별도 API Key가 없어도 주요 화면을 사용할 수 있고, 운영자는 R2에 올린 seed DB와 아이콘 리소스로 배포와 복구를 단순화할 수 있습니다.

## 차별점

| 기존 접근의 한계 | 이 프로젝트의 관점 |
| --- | --- |
| Holodex 기능이 여러 엔드포인트와 옵션으로 나뉘어 있음 | 라이브, 아카이브, 노래 DB, 통계를 하나의 탐색 UI로 통합 |
| 사용자가 화면을 열 때마다 외부 API를 호출하면 Rate Limit과 응답 지연이 누적됨 | 주요 기능을 DB 기반으로 전환해 반복 조회 비용을 서버 내부 조회로 흡수 |
...
| 검색 필터 | 아카이브 검색 |
| --- | --- |
| ![HoloSearch archive filter](docs/assets/readme/holo-filter.png) | ![HoloSearch archive search](docs/assets/readme/holo-archive.png) |

| 노래 DB 상세 | 방송 통계 |
| --- | --- |
| ![HoloSearch song detail](docs/assets/readme/holo-song-detail.png) | ![HoloSearch statistics dashboard](docs/assets/readme/holo-stats.png) |

현재 서비스는 아카이브 조건 검색, 콜라보 OR/AND 필터, 노래 DB 상세 탐색, 통계 조회를 한 흐름으로 제공합니다. 화면에서 보이는 채널 아이콘과 채널 인덱스는 정적 리소스 계층으로 분리해 로딩 부담을 줄였고, 영상 목록과 통계는 Holodex API를 직접 반복 호출하지 않고 DB-backed API에서 내려줍니다.
